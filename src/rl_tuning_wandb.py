#!/usr/bin/env python
"""
This file contains the code for hyperparamter tuning for a reinforcement learning agent to
complete proofs using Proverbot9001.
"""
...
def tuning(args) -> None:
    ...
    wandb.init()
    setattr(args, "gamma", wandb.config.gamma)
    setattr(args, "starting_epsilon", wandb.config.gamma)
    setattr(args, "batch_step", wandb.config.batch_step)
    setattr(args, "lr_step", wandb.config.lr_step)
    setattr(args, "batches_per_proof", wandb.config.batches_per_proof)
    setattr(args, "sync_target_every", wandb.config.sync_target_every)
    if args.splits_file:
        with args.splits_file.open("r") as f:
            project_dicts = json.loads(f.read())
        if any("switch" in item for item in project_dicts):
