...
class VNetwork:
    ...
    def __call__(self, obls: Union[Obligation, List[Obligation]]) -> torch.FloatTensor:
        ...
        if isinstance(obls, Obligation):
            obls = [obls]
        elif isinstance(obls, str):
            assert "Obligation" in obls
            obls = [eval(obls)]
        else:
        ...
        #print("here are my obls")
        #print(obls)
        new_obls = []
        for obl in obls:
            if isinstance(obl, Obligation):
                new_obls.append(obl)
            else:
                assert "Obligation" in obl
                new_obls.append(eval(obl))
        print("new_obls...")
        print(new_obls)
        encoded = self.obligation_encoder.obligations_to_vectors(
            [coq2vec.Obligation(list(obl.hypotheses), obl.goal) for obl in new_obls])\
...
def train_v_network(args: argparse.Namespace,
    ...
    for _batch_idx in range(args.batches_per_proof):
        ...
        if samples is None:
            return
        #inputs = [thing[0] for thing in samples]
        #action_records = [action_records for starting_obl, action_records in samples]
       ...
       # print(action_records[0][0])
       # print(action_records[0][1])
        #print(len(action_records[0][1]))
        #print(action_records[1][1])
        #print(len(action_records[1][1]))
        #print(action_records[2][1])
        #print(len(action_records[2][1]))
        #print(action_records[1])
        #print(action_records[2])
        ...
        with print_time("Computing outputs", guard=args.verbose >= 1):
            ...
            num_resting_obls = []
            #num_resulting_obls = [[len(resulting_obls) for action, resulting_obls in action_records]]
            #for starting_obls, action_records in samples:
                #print("...")
                #print(len(action_records))
                #print(action_records)
                #action, resulting_obls = action_records
                #print(len(resulting_obls))
                #print(resulting_obls)
            ...
            num_resulting_obls = [[len(action_records[1])] for starting_obl, action_records in samples]
            all_obls = []
...
class ReplayBuffer:
    ...
    def add_transition(self, transition: FullTransition) -> None:
        #print({transition,})
        setunion = {(transition[1], tuple(transition[2]))} | self._contents.get(str(transition[0]), (0, set()))[1])
        #print("here's my transition 1")
        #print(transition[1])
        #setunion = [transition[1], tuple(transition[2])]
        #getval = self._contents.get(str(transition[0]), None)
        if getval:
            setunion.append(getval[1])
        self._contents[str(transition[0])] = (self.window_end_position, setunion)
