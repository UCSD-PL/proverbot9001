...
def main():
    ...
    parser.add_argument("--verifyvval-resume", action="store_true")
    parser.add_argument("--state-dir", default="drl_vval_state", type=Path)
...
def verifyvval(args: argparse.Namespace, unique_id: uuid.UUID = uuid.uuid4()) -> None:
    print("Entering Verify Vval")
    ...
    print("Num tasks in file is",len(all_tasks))
    print("Num tasks Done is",num_tasks_done)
    ...
    if num_workers_actually_needed > 0:
        ...
        gammaargs = f"--gamma {args.gamma}"
    
    
        with open("submit/submit_multi_eval_jobs.sh","w") as f:
            submit_script = f"""#!/bin/bash
#
#SBATCH --job-name=Rl_verifyvval_{unique_id}
{partition_part}
#SBATCH --mem={args.mem}
...
python -u src/distributed_rl_verifyvval_cluster_worker.py {supervised_weights_arg} {coq2vecweightsarg} \
    compcert_projs_splits.json {prelude_arg} {backend_arg} {output_arg} {task_file_arg} \
     {verbosity_arg} {predictionarg} {setswitchargs} {proofrelevantargs} {maxsertopworkersarg} {statedirargs} \
     {blacklistedtacticsarg} -s {args.steps_per_episode} {gammaargs}
"""
    ...
    with tqdm(desc="Jobs finished", total=total_num_tasks, initial=jobs_done, dynamic_ncols=True,position=0,leave=True) as bar :
        while True :
            ...
            if jobs_done == total_num_tasks:
                print(jobs_done, total_num_tasks)
                break

def compute_average_step_err(args: argparse.Namespace) -> None:
    if args.tasks_file :
        vval_err_sum = 0
        total_num_jobs = 0
        for finished_file in glob(str(args.state_dir / "finished-*.txt")):
            with open(finished_file, 'r') as f:
                for line in f:
                    vval_err_sum += float(line)
                    total_num_jobs += 1
        print(f"Vval Average Error : {vval_err_sum/total_num_jobs}")
    else :
        raise(ValueError, "Not yet compatible for non-RLTask Jobs")
