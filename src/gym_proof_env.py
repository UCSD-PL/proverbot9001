...
#import search_strategies as srch_strat
...
#from search_worker import ReportJob as ReportJob
...
class ProofEnv(gym.Env):
    ...
    def _reset_to_start_of_file(self) :
        ...
        self.commands = load_commands(os.path.join(self.prelude, self.proof_files[self.proof_file_index]), progress_bar=True)
...
class FastProofEnv(gym.Env):
    def __init__(self, proof_files: List[str], prelude: str,
        ...
        print("made weightsfile, making pipes and children now")
        self._create_pipes_and_children()
        print("setting max term length")
        self.max_term_length = max_term_length
        print("setting state encoder")
        self.state_encoder = self._get_state_encoder()
        print("setting proofs done")
        self.proofs_done: List[ReportJob] = []
    ...
    def _create_pipes_and_children(self) :
        ...
        print("got context, entering into loop")
        for i in range(self.num_check_engines) :
