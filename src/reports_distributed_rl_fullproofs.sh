...
prelude="CompCert"
lr="1.8e-6"
job_name="without_subproof_filtered"
gamma=0.85
coq2vecweightspath=data/term2vec-weights-59.dat
width=5
train_path=data/compcert_train_tasks_paper_filtered_no_subproofs.json
resume=no
...
srun --pty -p gpu --mem=8G --time=8:00:00 --gres=gpu:1 python src/supervised_v_values.py \
        ...
        --learning-rate-step=972   --gamma=$gamma --num-epochs=1000 --hidden-size 128 --num-layers 2 --batch-size=2
