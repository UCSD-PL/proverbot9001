...
def Bandit(T_max, batch_size, args) :
	...
	tactic_space_model = fasttext.train_unsupervised(args.proof_file.path, model='cbow', lr = 0.1,epoch = 20)
	...
	while T <= T_max :
		...
		if T%update_every == 0 :
			for _ in tqdm.tqdm(range(200)) :
				...
				optimizer.step()
				if args.wandb_log :                
					wandb.log({"Agent Loss" : loss})

			
		if T%100 == 0 :
			torch.save(agent_model, "data/nn_bandit_model.model")
