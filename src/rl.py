...
from rl_search_worker import ReportJob, Worker, get_predictor
...
class ReinforcementWorker(Worker):
    ...
    def run_job_reinforce(self, job: ReportJob, epsilon: float, restart: bool = True) -> None:
        ...
        job_name = job.module_prefix + coq_serapy.lemma_name_from_statement(
            job.lemma_statement)
        print("working in run job reinforce on job name ...")
        print(job_name)
...
def reinforce_jobs(args: argparse.Namespace) -> None:
    ...
    if args.tasks_file:
        ...
        for task in partial_tasks:
            partial_task_job = ReportJob(project_dir=".", filename=task['src_file'], module_prefix=task['module_prefix'], lemma_statement=task['proof_statement'], tactic_prefix=task['tactic_prefix'])
            jobs.append(partial_task_job)
    

    with ReinforcementWorker(args, predictor, v_network, switch_dict,
                             initial_replay_buffer = replay_buffer) as worker:
        print("new Reinforcement worker")
        if args.interleave:
        ...
        for step, task in enumerate(tqdm(tasks[steps_already_done:]), start=steps_already_done+1):
