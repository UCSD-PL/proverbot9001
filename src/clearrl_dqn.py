...
class Agent(nn.Module):
    ...
    def forward(self, next_states) -> torch.Tensor:
        encoded_next_states = torch.cat([self.state_encoder(state) for state in next_states], dim=0)
...
if __name__ == "__main__":
    ...
    q_network = Agent(envs).to(device)
    optimizer = optim.Adam(q_network.parameters(), lr=args.learning_rate)
    target_network = Agent(envs).to(device)
