...
def parse_args():
        ...
        parser.add_argument("--wandb-project-name", type=str, default="rlprover",
                help="the wandb's project name")
        parser.add_argument("--wandb-entity", type=str, default="zhannakaufma",
...
class Agent(nn.Module):
        ...
        def forward(self, next_states) -> torch.Tensor:
                ...
                with open("/home/zhannakaufma_umass_edu/work/proverbot9001/state_scores.txt", "a") as f:
                    f.write("my state scores are ...\n")
                    f.write(str(state_scores))
                    f.write("\n")
                    f.write("softmax...\n")
                    f.write(str(self.softmax(state_scores)))
                    f.write("\n")
                    f.close()
...
def main():
        ...
        resumefile_path = Path(f"/home/zhannakaufma_umass_edu/work/proverbot9001/runs/{args.exp_name}-resumefile.dat")
        print("resumefile_path")
        print(resumefile_path)
                ...
                if global_step > args.learning_starts:
                        if global_step % args.train_frequency == 0:
                                ...
                                if global_step % 100 == 0:
                                        ...
                                        with open("/home/zhannakaufma_umass_edu/work/proverbot9001/q_values.txt", "a") as f:
                                            f.write("global step\n")
                                            f.write(str(global_step))
                                            f.write("\n")
                                            f.write("the q values\n")
                                            f.write(np.array2string(q_values))
                                            f.write("\n")
                                            f.write("the old value q_network value\n")
                                            f.write(str(old_val.mean().item()))
                                            f.write("\n")
                                            #f.write("the reachable states\n")
                                            #print(env.reachable_states, file=f)
                                            f.write("the target ... \n")
                                            f.write(str(td_target))
                                            f.write("\n")
                                            f.write("the reachable states\n")
                                            f.write('{}\n'.format(env.reachable_states))
                                            f.write("the observations\n")
                                            f.write(str(data.observations))
                                            f.write("\n")
                                            f.close()
