...
def parse_args():
    ...
    parser.add_argument('--weightsfile', default = "data/polyarg-weights.dat", type=Path)
    parser.add_argument('--proof_file', default="CompCert/common/Globalenvs.v",type=Path)
...
def make_env(args):
    ...
    return FastProofEnv(proof_files,args.prelude,weightsfile=args.weightsfile)
...
class Agent(nn.Module):
    ...
    def stateEncoder(self, state : ProofContext) :
        return self.CoqContextEncoder.obligation_to_vector(state.fg_goals[0].goal)
...
if __name__ == "__main__":
    ...
    # rb = ReplayBuffer(
    #     args.buffer_size,
    #     env.single_observation_space, #TODO: env observation space should be decided by encoder from gymnasium.spaces.space import Space
    #     env.single_action_space, #TODO: 
    #     device, 
    ...
    print("states returned by reset\n>>",states)
    print("#########")
    print("infos returned by reset\n>>",infos)
    for global_step in range(args.total_timesteps):
        # ALGO LOGIC: put action logic here
        epsilon = linear_schedule(args.start_e, args.end_e, args.exploration_fraction * args.total_timesteps, global_step)
        if random.random() < -1: # TODO Need to check how to select random actions
            # actions = np.array([envs.single_action_space.sample() for _ in range(envs.num_envs)]) # TODO: change envs to env,sample 
        ...
        next_obs, rewards, dones, infos = env.step(actions) # TODO: change envs to env
        print(infos)
        ...
        for idx, d in enumerate(dones):
            if d:
                real_next_obs[idx] = infos[idx]["terminal_observation"] #TODO:check 'terminal observation'
