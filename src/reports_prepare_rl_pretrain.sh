...
python src/distributed_get_task_initial_states.py $train_path output/rl/initial_states.json --state-dir output/rl/state_dirs/prepare_init_state --num-workers 32 
...
srun --pty -p gpu --mem=8G --time=1:00:00 --gres=gpu:1 python src/supervised_v_values.py \
        ...
        --prelude=CompCert --mode=train --supervised-weights=data/polyarg-weights-develop.dat -l 7e-4 --learning-rate-decay=.57 \
