...
def select_action(agent_model, env, predictor, epsilon, device) :
...
def NN_VLearning(T_max, gamma, args) :
	...
	if args.use_fast_check :
		env = FastProofEnv(proof_files, args.prelude, args.wandb_log, text_based_state = True, num_check_engines = args.max_attempts)
	else :
		env = ProofEnv(proof_files, args.prelude, args.wandb_log, text_based_state = True)
	...
	while T <= T_max :
		...
		prediction,_,agent_info = select_action(agent_model, env, predictor, curr_epsilon, device)
		if prediction == None :
