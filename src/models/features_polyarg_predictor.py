...
class FeaturesPolyargPredictor(
    ...
    def getAllPredictionIdxs(self, context: TacticContext
        ...
        goal_arg_values = self.goal_token_scores( #HERE
        ...
        if len(tokenized_premises[0]) > 0:
            hyp_arg_values = self.hyp_name_scores( #HERE
    ...
    def getAllPredictionIdxs_batch(self, contexts: List[TacticContext],
        ...
        for (stem_certainties, stem_idxs,
            ...
            if len(tokenized_premises) > 0:
                premise_arg_values = self.hyp_name_scores( # HERE
                    stem_idxs,
                    tokenized_goal,
                    tokenized_path,
                    tokenized_premises,
    ...
    def goal_token_scores(self, model: FeaturesPolyArgModel, args: argparse.Namespace,
        ...
        expanded_tokenized_paths = tokenized_paths.view(batch_size, 1, goal_len)\
            .expand(-1, stem_width, -1).contiguous()\
            .view(batch_size * stem_width, goal_len)
        #expanded_goals_and_paths = torch.cat((expanded_tokenized_goals, expanded_tokenized_paths), dim=1) #HERE
        unmasked_probabilities_pre = model.goal_args_model( stem_idxs.view(batch_size * stem_width), expanded_tokenized_goals)#[:,-(expanded_tokenized_goals.size()[1] + 1):,:] #HERE
    ...
    def hyp_name_scores(self,
        ...
        encoded_goals = self._model.goal_encoder(goals_tensor)#[:,-(goals_tensor.size()[1] + 1):] #HERE
        #encoded_goals_two = self._model.goal_encoder(LongTensor([tokenized_goal]))
    ...
    def _getBatchPredictionLoss(self, arg_values: Namespace,
        ...
        goal_arg_values = self.goal_token_scores(model, arg_values, mergedStemIdxsT, tokenized_goals_batch, tokenized_paths_batch, maybe_cuda(goal_masks_batch)) # HERE - pass paths tensor
        paths_and_goals_tensor = torch.cat((tokenized_goals_batch, tokenized_paths_batch), dim=1)
        encoded_goals = model.goal_encoder(tokenized_goals_batch)#[:,-(tokenized_goals_batch.size()[1] + 1):]  #HERE
        #print("size")
        #print(encoded_goals.size())
        
        #encoded_goals_two = encoded_goals[:,-(tokenized_goals_batch.size()[1] + 1):] # HERE - pass paths tensor 
        #encoded_goals_three = model.goal_encoder(tokenized_goals_batch) # HERE - pass paths tensor
        #print("my encoded goals size") 
        #print(encoded_goals_two.size())
        #print("their encoded goals size")
        #print(encoded_goals_three.size())
