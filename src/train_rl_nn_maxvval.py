...
def select_action(agent_model, env, predictor, epsilon, device = "cpu") :
	...
	vvals = get_vvals(next_states, agent_model, device)
	print(vvals)
...
def NN_VLearning(T_max, gamma, args) :
	...
	if args.use_fast_check :
		env = FastProofEnv(proof_files, args.prelude, args.wandb_log, state_type = "index", num_check_engines = args.max_attempts)
	else :
		env = ProofEnv(proof_files, args.prelude, args.wandb_log, state_type = "index")
	...
	while T <= T_max :
		...
		prediction,_,agent_info = select_action(agent_model, env, predictor, curr_epsilon, device = device)
		if prediction == None :
		...
		if T%1000 == 0 :
			torch.save(agent_model,"data/nn_maxvval_agent.model")
