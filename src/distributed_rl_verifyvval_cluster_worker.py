...
def main() -> None:
    ...
    verifyvval_worker(args,workerid,jobid)




def verifyvval_worker(args: argparse.Namespace, workerid: int, jobid: int) -> None:
    ...
    v_network = VNetwork(args.coq2vec_weights, 0.0, 1, 1.0,"Adam")
    target_network = VNetwork(args.coq2vec_weights, 0.0, 1, 1.0, "Adam")
    target_network.obligation_encoder = v_network.obligation_encoder
    # else:
    #     # Giving dummy values for learning rate, batch step, and lr step because we
    #     # won't be training this network, so it won't matter.
    #     v_network = VNetwork(None, 0.0, 1, 1.0, "Adam")
    #     target_network = VNetwork(None, 0.0, 1, 1.0, "Adam")
    #     target_network.obligation_encoder = v_network.obligation_encoder
    ...
    while True :
        ...
        if not tactic_prefix_is_usable(task.tactic_prefix):
            ...
            with (args.state_dir / f"finished-{workerid}.txt").open('a') as f,FileLock(f):
                    print(step_error, file=f, flush=True)
