...
class ProofEnv(gym.Env) :
	...
	def goto_next_proof(self):
		...
		if self.proof_line_num >= len(self.commands):
			print("File done")
...
def select_action(s, agent_model, tactic_space_model, env, predictor, epsilon) :
	...
	return predictions,action_idx,state_action

	


def Qlearning(T_max, gamma, batch_size, args) :
	...
	while T <= T_max :
		...
		predictions, action_index,state_action = select_action(s, agent_model, tactic_space_model, env, predictor, curr_epsilon)
		prediction = predictions[action_index]
		...
		memory.add(state_action,action_index,episode_r, s_next)
		...
		for state_action, action, reward, nxt_state in mem_batch :    
			curr_q.append(get_qvals(state_action,agent_model, grad_tensor = True)[action])
			if done :
				y.append(reward)
			else :
				state_action, predictions = get_state_action(nxt_state, tactic_space_model, env, predictor)
				qvals = get_qvals(state_action, agent_model)
				y.append( reward + gamma * max( qvals ) )
		...
		if done :
			...
			optimizer.zero_grad()
			if args.wandb_log :
				wandb.log({"Episode Total Rewards DeepQLearning:":episode_r})
...
if __name__ == "__main__" :
	...
	Qlearning(T_max= total_num_steps, gamma = gamma, args = args, batch_size = batch_size)
