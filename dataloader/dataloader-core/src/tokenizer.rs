...
pub fn get_words_from_paths(string: &str) -> Vec<&str> {
    lazy_static! {
        static ref WORDS: Regex =
            Regex::new(&format!(r"({}|(\??([[:word:]'!#\.])+)", SYMBOLS_REGEX)).unwrap();
    }
    let s1 = string.split(' ');
    let s2 = s1.collect::<Vec<&str>>();
    let mut s3: Vec<&str> = Vec::new();
    for word in s2 {
        let pre_plus = word.split("|-path-|").collect::<Vec<&str>>()[0];
        if !pre_plus.is_empty() {
            if WORDS.is_match(pre_plus) {
                s3.push(pre_plus);
            }
        }
    }
...
pub fn get_paths_from_words(string: &str) -> Vec<&str> {
    lazy_static! {
        static ref WORDS: Regex =
            Regex::new(&format!(r"({}|(\??([[:word:]'!#\.])+)", SYMBOLS_REGEX)).unwrap();
    }
    ...
    let mut s3: Vec<&str> = Vec::new();
    for word in s2 {
        let pre_plus = word.split("|-path-|").collect::<Vec<&str>>()[1];
        if !pre_plus.is_empty() {
            s3.push(pre_plus);
        }
    }
